...
<div>
<div style="text-align:right; min-height:80px; vertical-align: bottom;">
  ...
  <div class="config_elements">
    ...
    <ul id="list_of_candidadates">
      <li v-for="key in sortedCandidateKeys(Object.keys(all_config.candidates))" :key="key">
        ...
        <img src="../assets/edit.svg" @click="editCandidate(key)" class="linker" /> {{all_config.candidates[key]}}
      </li>
    </ul>
  </div>
</div>
    ...
    <p>
      <ul id="list_of_users">
        <li v-for="(user, index) in all_config.users" :key="index">
          <b>{{user.displayName}}</b> <u class="link" @click="deleteUser(index)">Delete user</u>
          <ul>
  ...
  methods:{
    ...
    deleteUser: function(userUid){
      ...
      deregisterUser({
        ...
        if (message.data === "success"){
          if (this.admin_config.admin_uid == userUid){
            this.fb_signout();
          } else{
            this.get_all_config();
          }
    ...
    commitBaseConfigs: function(){
      ...
      db.ref("/config").update({
        ...
        this.get_all_config();
        this.$emit("BaseConfigUpdated", {"mainTitle": newTitle});
      }).catch(() => {
